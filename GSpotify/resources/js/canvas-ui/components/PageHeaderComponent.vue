<template>
    <div class="border-bottom" style="background-color:#1B1212;">
        <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-12">
            <nav class="navbar d-flex px-0 py-1 sticky-top">
                <router-link :to="{ name: 'posts' }" class="navbar-brand hover font-weight-bolder font-serif mr-3" style="color:#1DB954">
                    GSpotify
                </router-link>
                <div class="mr-auto border-left pl-1">
                    <router-link :to="{ name: 'tags' }" class="btn btn-link py-0 text-decoration-none text-white">
                        Nummers
                    </router-link>
                    <router-link :to="{ name: 'topics' }" class="btn btn-link py-0 text-decoration-none text-white">
                        Albums
                    </router-link>
                </div>
                ...
                <div v-if="user" class="dropdown ml-3">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                        ...
                        <a v-if="isAdmin" :href="`/${canvasPath}/tags`" class="dropdown-item">Albums</a>
                        <a v-if="isAdmin" :href="`/${canvasPath}/topics`" class="dropdown-item">Nummers</a>
                        <a :href="`/${canvasPath}/stats`" class="dropdown-item">Stats</a>
                        <div class="dropdown-divider" />
                        <a :href="`/${canvasPath}/settings`" class="dropdown-item">Settings</a>
                        <a href="" class="dropdown-item" @click.prevent="logout"> Uitloggen </a>
                    </div>
                </div>

                <a v-if="!user" :href="`/${canvasPath}/login`" class="btn btn-link text-decoration-none text-white">Inloggen</a>
            </nav>
        </div>
    </div>
</template>
